...
//   );
// }
"use client"; // Framer motion වැඩ කරන්න client component එකක් වෙන්න ඕනේ
...
export default function AboutSection() {
  // Animations සඳහා configurations
  const fadeInRight = {
  ...
  const containerVariants = {
    ...
    visible: {
      opacity: 1,
      transition: { staggerChildren: 0.2 } // එකින් එක animate වෙන්න
    }
  };
    ...
    <section className="section-space bg-white overflow-hidden">
      <div className="container-page grid gap-12 lg:grid-cols-2 lg:items-center">
        ...
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.3 }} // Scroll කරද්දී එකපාරක් පමණක් animate වේ
        ...
        >
          ...
          >
            {[
              ...
              <motion.div key={item} className="flex items-start gap-3" variants={itemVariants}>
                <CheckCircle2 className="mt-1 text-[#2F6DB3]" size={20} />
                <span className="text-slate-700">{item}</span>
              </motion.div>
            ))}
          ...
          <motion.div className="mt-8" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Link
              href="/about"
              className="inline-flex items-center gap-2 rounded-full bg-[#2F6DB3] px-6 py-3.5 font-semibold text-white shadow-lg transition hover:bg-[#275c98]"
            >
              Learn More About Us
            </Link>
          </motion.div>
          ...
          <div className="relative overflow-hidden rounded-[0.3rem] border border-white/60 bg-white/70 p-0.4 shadow-1xl backdrop-blur-xl">
            <div className="relative h-[420px] overflow-hidden rounded-[0.5rem] bg-gradient-to-br from-[#2f6db300] to-[#3f89dd]">
              ...
              <motion.div
                ...
                className="absolute inset-0 bg-center bg-no-repeat bg-contain mix-blend-screen"
                style={{ backgroundImage: "url('/images/world-map.webp')" }}
              />

              {/* Floating Card with a slight bounce */}
              <motion.div 
                initial={{ y: 20, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.5, duration: 0.6 }}
                className="absolute bottom-6 left-6 right-6 rounded-2xl bg-white/92 p-5 shadow-lg backdrop-blur"
              >
                <div className="text-sm font-semibold text-[#2F6DB3]">
                  10+ Years Experience
                </div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  Delivering consistent logistics performance with reliability and care.
                </p>
              </motion.div>
            </div>
          </div>
