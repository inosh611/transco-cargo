...
export default function QuotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!fullName || !email || !origin || !destination) {
        ...
        <div className="relative z-10 container-page py-28 md:py-36 lg:py-44" style={{ marginLeft: "15%" }}>
          <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-4xl">
            ...
            <motion.p variants={fadeInUp} className="mt-8 max-w-2xl text-lg md:text-xl leading-8 text-slate-200/90">
              We provide dependable freight, storage, and specialized cargo services designed to help businesses move faster and operate smarter.
            </motion.p>
          </motion.div>
        </div>
      ...
      <section className="section-space bg-white">
        <div className="container-page grid gap-16 lg:grid-cols-[1fr_1.1fr]">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer}>
            ...
            <motion.p variants={fadeInUp} className="mt-6 text-lg leading-8 text-slate-600">
              We make it easier for businesses and individuals to understand the right logistics option before shipping.
            </motion.p>
            ...
            <div className="mt-12 grid gap-6">
              {quoteSteps.map((step) => {
                const Icon = step.icon;
                return (
                  <motion.div key={step.title} variants={fadeInUp} whileHover={{ x: 10 }} className="flex gap-5 rounded-[2rem] border border-slate-100 bg-slate-50/50 p-6 transition-shadow hover:shadow-md">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#2F6DB3] to-[#3f89dd] text-white shadow-lg">
                      <Icon size={26} />
                    </div>
                    <div>
            ...
            {/* Service types banner - මේක කලින් මගහැරුණා */}
            <motion.div variants={fadeInUp} className="mt-12 rounded-[2rem] bg-slate-900 p-8 text-white shadow-2xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 -mt-4 -mr-4 h-24 w-24 rounded-full bg-[#F47B20]/20 blur-3xl group-hover:bg-[#F47B20]/40 transition-colors" />
              <div className="relative z-10">
                <div className="text-xs font-bold uppercase tracking-[0.2em] text-[#F47B20]">Available Service Types</div>
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          {/* RIGHT SIDE: THE FORM */}
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="rounded-[2.5rem] border border-slate-200 bg-white p-8 md:p-12 shadow-2xl shadow-slate-200/50">
            <h2 className="text-2xl font-black text-slate-900">Quote Request Form</h2>
            <p className="mt-2 text-slate-500">Fill in the details below to get started.</p>
            ...
            <form className="mt-10 space-y-6" onSubmit={handleSubmit}>
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Full Name *</label>
                  <input name="fullName" required disabled={isSending} type="text" placeholder="Your name" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Company Name</label>
                  <input name="companyName" disabled={isSending} type="text" placeholder="Your company" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                </div>
              </div>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Email Address *</label>
                  <input name="email" required disabled={isSending} type="email" placeholder="email@example.com" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Phone Number</label>
                  <input name="phone" disabled={isSending} type="text" placeholder="+61 X XXX XXX" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                </div>
              </div>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <MapPinned className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input name="origin" required disabled={isSending} type="text" placeholder="City or Port" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-4 pl-12 pr-5 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Destination *</label>
                  <div className="relative">
                    <MapPinned className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input name="destination" required disabled={isSending} type="text" placeholder="City or Port" className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-4 pl-12 pr-5 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Shipment Type</label>
                  <select name="shipmentType" disabled={isSending} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all appearance-none cursor-pointer">
                    <option>Air Freight</option>
                    <option>Sea Freight</option>
                    <option>Road Freight</option>
                    <option>Warehousing</option>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 ml-1">Cargo Type</label>
                  <input name="cargoType" disabled={isSending} type="text" placeholder="e.g. Electronics" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-bold text-slate-700 ml-1">Shipment Details</label>
                <textarea name="details" disabled={isSending} rows={4} placeholder="Dimensions, weight, quantity..." className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 outline-none focus:bg-white focus:ring-2 focus:ring-[#2F6DB3]/20 focus:border-[#2F6DB3] transition-all" />
              </div>
              ...
              <motion.button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[#F47B20] py-5 font-bold text-white shadow-xl transition-all hover:bg-[#de6f1d] disabled:bg-slate-400"
              >
                {isSending ? (
